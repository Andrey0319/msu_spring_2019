...
int main()
{
	...
	uint64_t* mas_1 = new uint64_t[block_size];
	uint64_t* mas_2 = new uint64_t[block_size];
	uint64_t delta = nums_count_f_in % block_len;

	FILE* f_in = fopen("input.bin", "rb");
	FILE* f_out = fopen("temp.bin", "wb");
	for (uint64_t i = 0; i < count_blocks; i++)
	{
		fread(mas_1, sizeof(uint64_t), block_len, f_in);
		if (i == count_blocks - 1 && delta)
		{
			for (uint64_t j = delta; j < block_len; j++)
				mas_1[j] = -1;
		}
		qsort(mas_1, block_len, sizeof(uint64_t), comparison);
		fwrite(mas_1, sizeof(uint64_t), block_len, f_out);
	}
	fclose(f_in);
	fclose(f_out);
	...
	FILE* f = fopen("temp.bin", "r+b");
	uint64_t s = count_blocks;
	while (s > 1)
	{
		thread m1(find_left, mas_1, f, s, count_blocks, half_block_size, half_block, 
			block_len, std::ref(mx), std::ref(flag), std::ref(cv));
		thread m2(find_right, mas_2, f, s, count_blocks, half_block_size, half_block, 
			block_len, std::ref(mx), std::ref(flag), std::ref(cv));
		...
		{
			fseek(f, (count_blocks - s) * half_block_size + i * block_size, SEEK_SET);
			fread(mas_1, sizeof(uint64_t), block_len, f);
			qsort(mas_1, block_len, sizeof(uint64_t), comparison);
			fseek(f, (count_blocks - s) * half_block_size + i * block_size, SEEK_SET);
			fwrite(mas_1, sizeof(uint64_t), block_len, f);
		}
	}
	FILE* f_res = fopen("output.bin", "wb");
	fseek(f, 0, SEEK_SET);
	for (uint64_t i = 0; i < count_blocks; i++)
	{
		fread(mas_1, sizeof(uint64_t), block_len, f);
		if (i == count_blocks - 1 && delta)
		{
			fwrite(mas_1, sizeof(uint64_t), delta, f_res);
		}
		else
		{
			fwrite(mas_1, sizeof(uint64_t), block_len, f_res);
		}
	}
	fclose(f_res);
	fclose(f);
...
int comparison(const void* a, const void* b)
{
	uint64_t arg1 = *static_cast<const int*>(a);
	uint64_t arg2 = *static_cast<const int*>(b);
	...
	std::mutex& m, bool& flag, std::condition_variable& cv)
{
	...
	for (uint64_t i = 1; i < s; i++)
	{
		...
		qsort(mas, block_len, sizeof(uint64_t), comparison);
		fseek(f, (count_blocks - s + 2 * i) * half_block_size, SEEK_SET);
