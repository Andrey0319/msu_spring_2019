...
public:
	...
	Array_Controller(uint64_t size) : size(size)
	{
		mas = new uint64_t[size];
	}
...
int main()
{
	...
	Array_Controller mas_1(block_len);
	Array_Controller mas_2(block_len);
	...
	{
		fread(mas_1.mas, sizeof(uint64_t), block_len, f_in.f);
		if (i == count_blocks - 1 && delta)
		{
			for (uint64_t j = delta; j < block_len; j++)
				mas_1.mas[j] = -1;
		}
		sort(mas_1.mas, mas_1.mas + block_len - 1);
		fwrite(mas_1.mas, sizeof(uint64_t), block_len, f_temp.f);
	}
	...
	while (s > 1)
	{
		thread m1(find_left, mas_1.mas, f_temp.f, s, count_blocks, half_block_size, half_block,
			block_len, std::ref(mx), std::ref(flag), std::ref(cv));
		thread m2(find_right, mas_2.mas, f_temp.f, s, count_blocks, half_block_size, half_block,
			block_len, std::ref(mx), std::ref(flag), std::ref(cv));
		...
		for (uint64_t i = 0; i < s; i++)
		{
			...
			fread(mas_1.mas, sizeof(uint64_t), block_len, f_temp.f);
			sort(mas_1.mas, mas_1.mas + block_len - 1);
			fseek(f_temp.f, (count_blocks - s) * half_block_size + i * block_size, SEEK_SET);
			fwrite(mas_1.mas, sizeof(uint64_t), block_len, f_temp.f);
		}
	...
	for (uint64_t i = 0; i < count_blocks; i++)
	{
		...
		if (i == count_blocks - 1 && delta)
		{
			fwrite(mas_1.mas, sizeof(uint64_t), delta, f_out.f);
		}
		else
		{
			fwrite(mas_1.mas, sizeof(uint64_t), block_len, f_out.f);
		}
	}
