...
long long read_number(int &index, int &stop, char* args, int max_index);
char read_operator (int &index, int &stop, char* args, int max_index);
...
long long add(int &index, int arg1, char op, int neg1, int &stop, char* args, int max_index);
long long mul(int &index, int arg1, char op, int &neg, int &stop, char* args, int max_index);
...
long long add(int &index, int neg1, int &stop, char* args, int max_index)
{
    long long arg2;
    char op;
    int neg = 1;
...
int main(int argc, char* argv[])
{
    ...
    char* args = (char*)malloc(strlen(argv[1]));   //stop == 0  - continue calculating
    int i = 0, j = 0, stop = 0, index = 0;         //stop == 1  - end of expression
    long long res;                                 //stop == 2  - error

                                             //translating argv to args
    for (int i = 0; i < argc; i++)
    {
        j = 0;
        while (argv[i][j] != '\0' && i != 0)
        {
            args[index + j] = argv[i][j];
            j++;
        }
        index += j;
    }
